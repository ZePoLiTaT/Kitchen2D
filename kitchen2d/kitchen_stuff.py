...
class Kitchen2D(b2WorldInterface):
    ...
    def gen_liquid_in_cup(self, cup, N, userData='water'):
        ...
        # if grid_x*grid_y > N:
            # print('Liquid overflow?: ', grid_x*grid_y, N)
    ...
    def gen_liquid(self, position, usr_w, usr_d, N, userData='water'):
        '''
        Generate N liquid particles named userData uniformly in an empty cup.
        '''
        #import pdb; pdb.set_trace()

        grid_x = (usr_w - usr_d*2)/ self.liquid.radius / 2
        grid_y = (usr_w - usr_d) / self.liquid.radius / 2
        # if grid_x*grid_y > N:
            # print('Liquid overflow?: ', grid_x*grid_y, N)
        #assert grid_x*grid_y > N, 'cup cannot hold {} particles'.format(N)
        for i in range(N):
            x, y = i % grid_x, i / grid_x
            pos = (self.liquid.radius*(2*x+1)+usr_d, self.liquid.radius*(2*y+1)+usr_d)
            self.liquid.make_one_particle(position + pos, userData)


class WaterMaze2D(b2WorldInterface):
    def __init__(self, do_gui, sink_d, sink_pos_x, sink_pos_y,
                 left_table_width, right_table_width, faucet_h, faucet_w, planning=True,
                 obstacles=None, save_fig=False, liquid_name='water', liquid_frequency=0.2, overclock=None):
        '''
        Args:
            sink_d: table/sink thickness.
            ...
            this feature is not used.
        '''
        super(WaterMaze2D, self).__init__(do_gui, save_fig=save_fig)
        self.planning = planning
        world = self.world
        self.liquid = Liquid(world, liquid_name=liquid_name, liquid_frequency=liquid_frequency)
        self.disable_liquid = False
        self.base = world.CreateStaticBody(
            position=(sink_pos_x, -0),
            shapes=[
                    b2PolygonShape(vertices=[(-left_table_width, 0),
                                             (right_table_width, 0),
                                             (right_table_width, 0-sink_d),
                                             (-left_table_width, 0-sink_d)])
                    ],
            userData='countertop'
        )
        self.faucet = world.CreateStaticBody(
            position=(sink_pos_x, sink_pos_y),
            shapes=[b2PolygonShape(vertices=[(-faucet_w, -faucet_h),
                                             ( faucet_w, -faucet_h),
                                             ( faucet_w,  faucet_h),
                                             (-faucet_w,  faucet_h)])],
            userData='faucet')
        self.faucet_location = (sink_pos_x, sink_pos_y-1)

        self.add_obstacles(obstacles)
        self.overclock = overclock
        self.enabled_faucet = False
    ...
    def add_obstacles(self, obstacles=None):
        ...
        for obs in obstacles:
            ...
            if len(obs) == 2:
                ...
                base = self.world.CreateStaticBody(
                    position=obs[0],
                    shapes=[b2PolygonShape(vertices=obs[1:])],
                    userData='countertop')
    ...
    def step(self):
        ...
        for liquid generated from the faucet.
        '''
        if (not self.planning) and (not self.disable_liquid) and self.enabled_faucet:
            pos = np.random.normal(0, 0.1, size=(2)) + \
                np.array(self.faucet_location)
            self.liquid.make_particles(pos)
        super(WaterMaze2D, self).step()
