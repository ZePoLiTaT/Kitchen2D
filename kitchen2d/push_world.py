    ...
    def draw(self, bodies, bg_color=(64, 64, 64, 0)):
        ...
        def my_draw_polygon(polygon, body, fixture):
            vertices = [(self.screen_origin + body.transform*v)
                        * self.PPM for v in polygon.vertices]
        ...
        def my_draw_circle(circle, body, fixture):
            position = (self.screen_origin +
                        body.transform*circle.pos)*self.PPM
...
class end_effector:
    def __init__(self, b2world_interface, init_pos, base, init_angle, hand_shape='rectangle', hand_size=(0.3, 1)):
        ...
        self.hand = world.CreateDynamicBody(
            position=init_pos, angle=init_angle)
...
def construct_scene(table_width, table_length, b2world_interface, thing_shape, thing_size, thing_friction, thing_density, base_friction, obj_loc):
    world = b2world_interface.world
    ...
    # add sensor representing goal region
    base.CreateFixture(  # postiion defaults to origin
        shape=b2PolygonShape(box=(table_length, table_width)),
        isSensor=True,
    )
